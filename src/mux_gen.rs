...
/// Banner prepended to generated `config.toml` so operators can see *why*
/// `heartbeat_enabled` is off-by-default before they edit the file.
const DAEMON_CONFIG_BANNER: &str = "\
# Generated by `rust-mux` — daemon manifest of upstream MCP servers.
#
# Heartbeat policy:
...
fn render_daemon_config_toml(daemon_cfg: &Config) -> Result<String> {
    let body = toml::to_string_pretty(daemon_cfg).context("serialize daemon config.toml")?;
    Ok(format!("{DAEMON_CONFIG_BANNER}{body}"))
}
...
pub fn build_mux_outputs(
    ...
    let config_toml = render_daemon_config_toml(&daemon_cfg)?;
    ...
    for svc in services {
        ...
        cfg.servers.insert(
            ...
            ServerConfig {
                ...
                // global `true` triggers endless restart loops. Flip per-service
                // for backends that explicitly support heartbeat.
...
mod tests {
    ...
    #[test]
    fn daemon_config_disables_heartbeat_by_default() {
        // Mirror of `scan::tests::build_manifest_disables_heartbeat_by_default`
        // for the `mux_gen` daemon-config path. The two generators must agree:
        // any wizard-driven safe-path or per-client output ships heartbeat
        // off, with the rationale banner threaded into the artifact itself.
        let dir = tempdir().expect("tempdir");
        let mux_dir = dir.path().join("mux");
        let outputs =
            build_mux_outputs(&one_service(), &mux_dir, "rust-mux-proxy", &[]).expect("build");

        assert!(
            outputs.config_toml.contains("heartbeat_enabled = false"),
            "daemon config must default heartbeat_enabled = false:\n{}",
            outputs.config_toml
        );
        assert!(
            !outputs.config_toml.contains("heartbeat_enabled = true"),
            "daemon config must not advertise heartbeat_enabled = true:\n{}",
            outputs.config_toml
        );
        assert!(
            outputs.config_toml.contains("Heartbeat policy"),
            "daemon config.toml must carry the heartbeat-policy banner:\n{}",
            outputs.config_toml
        );
    }
