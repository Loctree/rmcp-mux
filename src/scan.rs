    ...
    for svc in merged.services {
        ...
        cfg.servers.insert(
            ...
            ServerConfig {
                ...
                // binaries) do not implement rust-mux ping/pong probes, so a
...
/// Banner prepended to TOML manifests written by `rust-mux scan --manifest`.
...
# Generated by `rust-mux scan` — daemon manifest of upstream MCP servers.
#
# Heartbeat policy:
...
#   binaries) do not respond to rust-mux heartbeats and would be restarted in
#   a loop. Flip to `true` per-service for backends you have verified support
#   the rust-mux heartbeat probe.
\n";
...
mod tests {
    ...
    fn build_manifest_disables_heartbeat_by_default() {
        ...
        // = true, which forced rust-mux to ping every upstream MCP. Most MCP
        // servers (npx wrappers, custom stdio binaries) do not respond to
        // rust-mux probes, so the daemon spiralled into restart loops. The
