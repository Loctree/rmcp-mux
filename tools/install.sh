...
info() { printf "[rust_mux] %s\n" "$*"; }
warn() { printf "[rust_mux][warn] %s\n" "$*" >&2; }
...
info "Installing rust_mux from $REPO_URL (ref: $MUX_REF)"
...
cargo install --git "$REPO_URL" --rev "$MUX_REF" $lock_flag --force rust_mux >/dev/null

installed_bin="$CARGO_BIN/rust_mux"
if [[ ! -x $installed_bin ]]; then
  warn "rust_mux binary not found at $installed_bin after install";
  exit 1;
fi

mkdir -p "$INSTALL_DIR"
wrapper="$INSTALL_DIR/rust_mux"
cat >"$wrapper" <<WRAP
#!/usr/bin/env bash
exec "$installed_bin" "\$@"
WRAP
...
ensure_path_line() {
  ...
  if grep -q "rust_mux installer" "$file"; then
    return
  fi
...
case ":$PATH:" in
  *":$INSTALL_DIR:"*) :;;
  *) warn "rust_mux wrapper dir not in PATH; adding to ~/.zshrc"; ensure_path_line "$HOME/.zshrc";;
esac

info "Done. Try: rust_mux --socket /tmp/mcp.sock --cmd npx -- @modelcontextprotocol/server-memory --tray"
